...
const EMPTY = {
  ...
  priceLakh:"",facing:"အရှေ့",road:"",tag:"",listingType:"sale",status:"available",note:"",images:[],
  agentName:"",agentPhone:"",
};

const PHONE_FIELDS = ["street", "road", "tag", "note"];

function statusLabel(status, listingType) {
  if (listingType === "rent") {
    return { available:"ငှားရန်ရှိ", sold:"ငှားပြီး", pending:"ဆိုင်းငံ့" }[status] ?? status;
  }
  return { available:"ရောင်းရန်ရှိ", sold:"ရောင်းပြီး", pending:"ဆိုင်းငံ့" }[status] ?? status;
}
const STATUS_COLOR = {
...
export default function Admin() {
  ...
  useEffect(() => {
    if (status === "authenticated" && searchParams.get("add") === "1") {
      openAdd();
  ...
  async function handleImageUpload(e) {
    ...
    if (slots <= 0) { alert("ဓာတ်ပုံ ၆ ပုံသာ တင်နိုင်သည်"); return; }
    ...
    if (valid.length < toUpload.length) alert("ဓာတ်ပုံ အချို့ တင်မရပါ");
  ...
  return (
    <div className="app">
                  ...
                  <div style={{ flex:1, padding:"12px 14px", minWidth:0 }}>
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:8,
                      ...
                      <span style={{
                        ...
                        {statusLabel(p.status, p.listingType)}
                      </span>
                    </div>
      ...
      {/* Add / Edit bottom sheet — portalled to body to escape .app stacking context */}
          ...
          <div style={{
            ...
            <div style={{ padding:"16px 20px 40px" }}>
              ...
              <div style={{ display:"flex", gap:8, marginBottom:16 }}>
                {[["sale","ရောင်းမည်"],["rent","အဌားမည်"]].map(([val, label]) => (
                  <button key={val} onClick={() => set("listingType", val)} style={{
                    flex:1, padding:"10px", borderRadius:10, border:"none", fontSize:14,
                    fontWeight:700, cursor:"pointer",
                    fontFamily:"var(--font-myanmar, system-ui)",
                    background: form.listingType === val
                      ? "linear-gradient(135deg,#E0A33B,#F4B942)"
                      : "#191C25",
                    color: form.listingType === val ? "#1A1206" : "#6B7280",
                    border: form.listingType === val ? "none" : "1px solid #2C313C",
                  }}>{label}</button>
                ))}
              </div>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                ...
                <Field label={form.listingType === "rent" ? "ငှားဈေး (ကျပ်/လ) *" : "ဈေး (သိန်း) *"}>
                  <input style={inp} type="number" value={form.priceLakh}
                    placeholder={form.listingType === "rent" ? "250000" : "250"}
              ...
              <Field label="အခြေအနေ">
                <select style={inp} value={form.status}
                  ...
                  <option value="available">{form.listingType === "rent" ? "ငှားရန်ရှိ" : "ရောင်းရန်ရှိ"}</option>
                  <option value="sold">{form.listingType === "rent" ? "ငှားပြီး" : "ရောင်းပြီး"}</option>
              ...
              <Field label="မှတ်ချက်" error={phoneErrors.note}>
                <textarea style={{ ...inp, resize:"none", borderColor: phoneErrors.note ? "#f87171" : "#2C313C" }}
                  ...
                  placeholder={form.listingType === "rent" ? "ဥပမာ ၁ နှစ် (သို့) ၆လ စာ တင်ရမည်" : "ဥပမာ ဂရန်အမည်ပေါက်၊ ကင်းရှင်း"}
                  onChange={e => set("note", e.target.value)}/>
              </Field>

              {/* Agent info */}
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                <Field label="အကျိုးဆောင် နာမည်">
                  <input style={inp} value={form.agentName} placeholder="ဥပမာ ကိုမောင်"
                    onChange={e => set("agentName", e.target.value)}/>
                </Field>
                <Field label="အကျိုးဆောင် ဖုန်း">
                  <input style={inp} type="tel" value={form.agentPhone} placeholder="09xxxxxxxx"
